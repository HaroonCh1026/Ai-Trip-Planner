...
import { generateTripItinerary, refineTripItinerary, generateInsiderInsights } from '../services/gemini.service';
...
//
// "Same destination" check: a Pro user could refine their trip to a different
// destination, in which case the cached insights are stale. We compare
// trip.destination === insiderInsights.destination as a cheap freshness check.
export const getInsiderInsights = async (
  ...
  try {
    ...
    // this trip, return them instantly. The destination check guards against
    // the rare case where a refinement changed the trip's destination.
