...
export default function AdminSupport({ tickets, setTickets, onReload }) {
  ...
  return (
    ...
    <div className="anim-fadeIn">
      ...
      {/* Filters */}
      <div className="vai-admin-support-filters" style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
        <div className="vai-admin-search" style={{ position:"relative", flex:1, minWidth:220 }}>
        ...
        <div className="vai-admin-support-filter-chips" style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
      ...
      <div
        className="vai-admin-support-grid"
        style={{ display:"grid", gridTemplateColumns:activeTicket?"1fr 460px":"1fr", gap:20 }}
      >
        {/* Ticket List */}
        <div className="card" style={{ overflow:"hidden" }}>
          {filtered.length === 0 ? (
            ...
            <div
              ...
              className="vai-admin-ticket-row"
            ...
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <p style={{ fontSize:12, color:C.midGray, marginBottom:3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{ticket.email}</p>
                ...
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <span style={{ display:"inline-block", padding:"3px 10px", borderRadius:4, fontSize:11, fontWeight:600, background:STATUS_STYLE[ticket.status]?.bg, color:STATUS_STYLE[ticket.status]?.color, marginBottom:4, whiteSpace:"nowrap" }}>{ticket.status}</span>
                  <p style={{ fontSize:11, color:C.midGray, whiteSpace:"nowrap" }}>{fmtDate(ticket.createdAt)}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Thread Panel — becomes a fullscreen overlay on mobile via the
            vai-admin-thread-panel class + media query. */}
        {activeTicket && (
          <div className="card vai-admin-thread-panel" style={{ display:"flex", flexDirection:"column", height:620, overflow:"hidden" }}>
            {/* Panel header */}
            <div style={{ padding:"18px 20px", borderBottom:"1px solid rgba(255,255,255,0.06)", flexShrink:0 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
                <div style={{ minWidth:0, flex:1 }}>
                  <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:17, marginBottom:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{activeTicket.name}</h3>
                  <p style={{ fontSize:12, color:C.midGray, marginBottom:8, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{activeTicket.email}</p>
                ...
                <button
                  ...
                  style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", color:C.midGray, cursor:"pointer", fontSize:18, width:34, height:34, borderRadius:6, flexShrink:0, display:"inline-flex", alignItems:"center", justifyContent:"center" }}
                >
                  ✕
                </button>
              </div>
              {/* Status controls */}
              <div className="vai-admin-thread-actions" style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
                {["Open","In Progress","Closed"].filter(s=>s!==activeTicket.status).map(s=>(
                  <button key={s} onClick={()=>updateStatus(s)} style={{ padding:"6px 12px", background:"transparent", border:`1px solid ${STATUS_STYLE[s]?.color||C.midGray}`, borderRadius:4, color:STATUS_STYLE[s]?.color||C.midGray, cursor:"pointer", fontSize:11, fontFamily:"'DM Sans',sans-serif", whiteSpace:"nowrap" }}>
                    → {s}
                  </button>
                ))}
              </div>
              ...
              {(activeTicket.messages||[]).length === 0 && (
                <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    ...
                    <div style={{ padding:"10px 14px", borderRadius:"4px 12px 12px 12px", background:"rgba(255,255,255,0.06)", fontSize:13, lineHeight:1.6, wordBreak:"break-word" }}>{activeTicket.message}</div>
                  </div>
                </div>
              )}
              {(activeTicket.messages||[]).map((msg,i)=>(
                <div key={i} style={{ display:"flex", flexDirection:msg.sender==="admin"?"row-reverse":"row", gap:10, alignItems:"flex-start" }}>
                  ...
                  <div style={{ maxWidth:"75%", minWidth:0 }}>
                    ...
                    <div style={{ padding:"10px 14px", borderRadius:msg.sender==="admin"?"12px 4px 12px 12px":"4px 12px 12px 12px", background:msg.sender==="admin"?C.crimson:"rgba(255,255,255,0.06)", fontSize:13, lineHeight:1.6, wordBreak:"break-word" }}>
                      {msg.text}
                    </div>
                  </div>
                </div>
            ...
            {activeTicket.status !== "Closed" ? (
              <div style={{ padding:"14px 20px", borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", gap:10, flexShrink:0 }}>
                ...
                <button className="btn-primary" style={{ padding:"10px 18px", fontSize:13, flexShrink:0 }} onClick={sendMessage} disabled={sending||!msgText.trim()}>
                  {sending?"...":"Send"}
                </button>
              </div>
            ) : (
